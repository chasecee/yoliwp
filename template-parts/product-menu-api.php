...
if ( isset( $_COOKIE['Country'] ) && isset( $_COOKIE['Language'] ) ) {
	$server_url = $base + url . $_COOKIE['Country'] . '/' . $_COOKIE['Language'];
}
	...
	<div class="product-menu-col">
		...
		<ul>
			<?php foreach ( $item->products as $product ) { ?>
				<li><a href="products/<?php /** phpcs:ignore */ echo esc_attr( strtolower( $product->itemDescription ) ); ?>/?item_id=<?php echo $product->itemID; ?>"><?php echo esc_html( $product->itemDescription ); ?></a></li>
			<?php } ?>
			</ul>
		<?php } ?>
	</div>
